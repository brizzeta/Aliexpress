...
@Component({
  ...
  styleUrls: ['./processing-tab.component.scss']
})
export class ProcessingTabComponent implements AfterViewInit, OnInit {
  @ViewChild('slider', { static: false }) slider!: ElementRef;
  public showProcessingTab: boolean = true;
  ...
  private checkRoute(): void {
    ...
    // Показываем карточки обработанных заказов только если нет дочерних маршрутов (track, buy-again, noactive)
    this.showProcessingTab = !url.includes('track') && 
  ...
  onOutletActivated(component: any) {
    ...
    this.showProcessingTab = !this.isNoactiveRoute; // Скрываем основной контент, если загружен noactive
  }
}