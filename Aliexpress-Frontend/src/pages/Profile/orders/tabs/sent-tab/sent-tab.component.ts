...
export class SentTabComponent implements AfterViewInit, OnInit {
  @ViewChild('slider', { static: false }) slider!: ElementRef;
  public showSentTab: boolean = true;
  ...
  private checkRoute(): void {
    ...
    // Показываем карточки отправленных заказов только если нет дочерних маршрутов (track, buy-again, noactive)
    this.showSentTab = !url.includes('track') && 
  ...
  onOutletActivated(component: any) {
    ...
    this.showSentTab = !this.isNoactiveRoute; // Скрываем основной контент, если загружен noactive
  }
}